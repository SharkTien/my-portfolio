...
app.use(cors({
    origin: '*', // For production, you could restrict this to 'https://sharktien.github.io'
}));

app.use(express.json());

// System prompt injected from the server securely
...
app.post('/api/gemini', async (req, res) => {
    ...
    const { prompt, messagesHistory } = req.body;
    ...
    try {
        ...
        // Forward headers to stream chunk by chunk
...
// DeepSeek/OpenRouter Proxy Route (Streaming)
app.post('/api/deepseek', async (req, res) => {
    const apiKey = process.env.VITE_ROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY;
    if (!apiKey) {
    ...
    try {
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            headers: {
                ...
                "HTTP-Referer": "https://sharktien.github.io/my-portfolio", // Safely set by backend
                "X-Title": "Soi long cua Tien Huynh (Secure Backend Proxy)",
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                ...
                stream: true // Enable OpenRouter streaming
            })
        });
...
app.get('/api/health', (req, res) => {
    res.status(200).json({ status: 'Platform API Proxy running securely.' });
});
